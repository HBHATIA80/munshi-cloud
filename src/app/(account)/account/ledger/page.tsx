...
export default async function MyLedgerPage() {
  ...
  // color rule (customer view): payable (they owe the shop) = red, advance = green
  const balColor = bal >= 0 ? "var(--red, #c62828)" : "var(--green, #1a7f37)";
  return (
    <div className="panel">
      <div className="ph">
        <h3>My ledger</h3>
      ...
      <div className="tblw"><table className="t">
        ...
        <tbody>
          {!!party.open && <tr><td>—</td><td><b>Opening balance</b></td><td />
            ...
            <td className="num">{party.open < 0 ? fmt0(-party.open) : ""}</td>
            <td className={"num " + (party.open >= 0 ? "neg" : "pos")}>{fmt0(Math.abs(party.open))} {party.open >= 0 ? "Dr" : "Cr"}</td></tr>}
          {rows.map((r, i) => {
            ...
            return (<tr key={i}><td>{r.date}</td><td>{r.part}</td>
              ...
              <td className={"num " + (run >= 0 ? "neg" : "pos")} style={{ fontWeight: 700 }}>{fmt0(Math.abs(run))} {run >= 0 ? "Dr" : "Cr"}</td></tr>);
          })}
