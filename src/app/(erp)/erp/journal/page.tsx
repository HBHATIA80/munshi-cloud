...
export default async function JournalPage() {
  ...
  const [{ data: parties }, { data: jnls }] = await Promise.all([
    ...
    sb.from("vouchers")
      .select("no,date,party_id,total,narr")
      .eq("type", "journal")
      .order("date", { ascending: false }),
  ]);

  const legs = (jnls ?? []) as {
    no: string; date: string; party_id: string | null; total: number; narr: string | null;
  }[];
  ...
  return (
    <>
      <p className="mut" style={{ fontSize: 12, margin: "0 0 10px" }}>
        {transfers} transfer{transfers === 1 ? "" : "s"} · {legs.length} journal leg{legs.length === 1 ? "" : "s"} · page 1 of 1
      </p>
      <PartyJournal
