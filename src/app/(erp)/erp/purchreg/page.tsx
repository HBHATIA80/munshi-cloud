...
export default async function PurchRegPage({ searchParams }:
  ...
  const [{ count, data: vs }, { data: ps }] = await Promise.all([
    sb.from("vouchers").select("*").eq("type", "purchase")
  ...
  const qs = (over: Record<string, string>) => {
    ...
    return `/erp/purchreg?${p.toString()}`;
  };
